...
- **WHEN** project root contains a known tool directory (for example `.claude`, `.cursor`, `.codex`, `.opencode`, `.github`, `.gemini`, `.amazonq`, `.windsurf`, `.clinerules`, `.continue`, `.trae`, `.kiro`, `.qwen`, `.roo`, `.agent`, `.kilocode`)
