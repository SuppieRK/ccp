...
- When touching older non-BDD tests, prefer moving them toward the surrounding package style if that can be done safely in the same change.
...
- Filter-specific fixture and benchmark expectations belong in `docs/agent-rules/FILTERS.md` and `docs/agent-rules/BENCHMARKS.md`.
...
- Benchmark and replay coverage lives in `internal/benchmark`, `internal/replay`, `internal/lifecycle`, and `testdata/benchmarks/` depending on what changed.
- Lifecycle adapter tests should prefer shared family/conformance coverage, with bespoke adapter tests only where bespoke behavior remains.
...
- Treat the internal coverage gate enforced by `./scripts/validate.sh` as part of the testing contract, not an optional extra.
