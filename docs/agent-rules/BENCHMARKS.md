...
- When present, `output.txt` and `decisions.txt` are exact expectations checked against `verify-output.txt` and
...
- When a fixture drifts, promote `verify-output.txt` / `verify-decisions.txt` or regenerate with `ccp capture` instead
  of rewriting expectations from memory.
- Treat missing `gain.db`, missing `verify-output.txt`, output mismatches, decision mismatches, or any non-zero
