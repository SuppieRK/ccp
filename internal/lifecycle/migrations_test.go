...
var _ = Describe("lifecycle migrations", func() {
	...
	Describe("migrate-repo-local-cmdshape-gitignore", func() {
		It("migrates exact root gitignore entries after creating nested ignore state", func() {
			...
			Expect(os.Mkdir(filepath.Join(ctx.cwd, ".cmdshape"), 0o755)).To(Succeed())
			rootGitignore := filepath.Join(ctx.cwd, ".gitignore")
			Expect(os.WriteFile(rootGitignore, []byte("node_modules/\n.cmdshape\n.cmdshape/\n.cmdshape/**\n!.cmdshape/filters/\n"), 0o644)).To(Succeed())

			Expect(migrateRepoLocalCmdshapeGitignore(ctx)).To(Succeed())
			Expect(migrateRepoLocalCmdshapeGitignore(ctx)).To(Succeed())

			nested, err := os.ReadFile(filepath.Join(ctx.cwd, ".cmdshape", ".gitignore"))
		...
		It("noops outside git roots", func() {
			Expect(os.Mkdir(filepath.Join(ctx.cwd, ".cmdshape"), 0o755)).To(Succeed())
			rootGitignore := filepath.Join(ctx.cwd, ".gitignore")
			Expect(os.WriteFile(rootGitignore, []byte(".cmdshape\n"), 0o644)).To(Succeed())

			Expect(migrateRepoLocalCmdshapeGitignore(ctx)).To(Succeed())

			_, err := os.Stat(filepath.Join(ctx.cwd, ".cmdshape", ".gitignore"))
			Expect(err).To(MatchError(os.ErrNotExist))
			root, err := os.ReadFile(rootGitignore)
			Expect(err).NotTo(HaveOccurred())
			Expect(string(root)).To(Equal(".cmdshape\n"))
		})

		It("noops in git roots without cmdshape local state", func() {
			Expect(os.Mkdir(filepath.Join(ctx.cwd, ".git"), 0o755)).To(Succeed())
			rootGitignore := filepath.Join(ctx.cwd, ".gitignore")
			Expect(os.WriteFile(rootGitignore, []byte(".cmdshape\n"), 0o644)).To(Succeed())
		...
		It("does not edit the root gitignore when nested ignore ensure fails", func() {
			...
			if err := os.Symlink(outside, filepath.Join(cmdshapeDir, ".gitignore")); err != nil {
				Skip("symlink creation unavailable: " + err.Error())
			}

			err := migrateRepoLocalCmdshapeGitignore(ctx)
		...
		It("produces git-visible filters while ignoring generated state", func() {
			...
			cmdshapeDir := filepath.Join(ctx.cwd, ".cmdshape")
			Expect(os.MkdirAll(filepath.Join(cmdshapeDir, "filters"), 0o755)).To(Succeed())
			Expect(os.WriteFile(filepath.Join(ctx.cwd, ".gitignore"), []byte(".cmdshape\n"), 0o644)).To(Succeed())
			Expect(os.WriteFile(filepath.Join(cmdshapeDir, "gain.db"), []byte("metrics"), 0o644)).To(Succeed())
			Expect(os.WriteFile(filepath.Join(cmdshapeDir, "filters", "local.yaml"), []byte("version: 1\n"), 0o644)).To(Succeed())

			Expect(migrateRepoLocalCmdshapeGitignore(ctx)).To(Succeed())

			ignored := gitCheckIgnore(ctx.cwd, ".cmdshape/gain.db", ".cmdshape/.gitignore")
			Expect(ignored).To(ContainElement(".cmdshape/gain.db"))
			Expect(ignored).To(ContainElement(".cmdshape/.gitignore"))
			Expect(gitCheckIgnore(ctx.cwd, ".cmdshape/filters/local.yaml")).To(BeEmpty())
		})
	})
	...
	Describe("cleanup-legacy-project-init-state", func() {
		It("removes legacy init files while preserving gain db", func() {
			...
			initPath := filepath.Join(cmdshapeDir, "init.json")
			backupPath := filepath.Join(cmdshapeDir, "init.json.bak.123")
			gainPath := filepath.Join(cmdshapeDir, "gain.db")
