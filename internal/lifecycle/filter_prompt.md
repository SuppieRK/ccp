...
- If a matching global, home-scoped, or shipped filter already exists, copy it into `./.cmdshape/filters` first and edit that project-local copy.
- Do not edit global/home filters under `~/.config/cmdshape/filters` unless the user directly asks for a global filter change.
- Do not edit shipped built-in filters under `filters/` unless the user directly asks to change cmdshape's built-in filter set.
...
- Project-local filters remain inactive until the user approves their exact current bytes with `cmdshape filter trust`; never approve changed bytes without reviewing the complete source.

## Workflow

1. Inspect active filters with `cmdshape filter status`.
2. If `{{FILTER_ID}}` already exists outside `./.cmdshape/filters`, create `./.cmdshape/filters` and copy the existing YAML into `./.cmdshape/filters/{{FILTER_ID}}.yaml` before editing.
3. If no matching filter exists, run `cmdshape filter new {{FILTER_ID}}`.
4. Capture real command output with `cmdshape capture -- {{COMMAND_EXAMPLE}}`.
5. Edit only the project-local YAML filter and project-local `.mappings.yaml` as needed.
6. Run `cmdshape verify` or `cmdshape verify --dir <fixture-dir>`.
7. Compare `output.txt` with `verify-output.txt`, and inspect `verify-decisions.txt` when behavior is unclear.
8. After reviewing every project YAML file and `.mappings.yaml`, run `cmdshape filter trust` if the user wants this exact source activated.

## Performance-Guided Improvements

- Run `cmdshape filter performance --limit 30` to find local filter rows worth improving.
- For focused work on one command, run `cmdshape filter performance --tool <tool> --limit 30`. The `--tool` value is the invoked tool name, not necessarily the resolved filter id.
- Use `cmdshape filter performance --global --tool <tool> --limit 30` only when the user wants cross-workspace signal.
