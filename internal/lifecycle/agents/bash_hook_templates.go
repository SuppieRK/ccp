...
func bashBlockingHookScriptContent(agent, description, logFile, toolName, commandKeys, outputSnippet string) string {
	keyArgs := `"` + strings.Join(strings.Fields(commandKeys), `" "`) + `"`
	return "#!/bin/bash\n" +
		"# generated by ccp init for " + agent + "\n" +
		"# " + description + "\n\n" +
		"LOG_FILE=\"${TMPDIR:-/tmp}/" + logFile + "\"\n\n" +
...
TOOL="$(json_extract_string "$INPUT" tool_name toolName tool)"
if [[ -n "` + toolName + `" && "$TOOL" != "` + toolName + `" ]]; then
  log_hook "skip-wrong-tool"
  exit 0
fi

CMD="$(json_extract_string "$INPUT" ` + keyArgs + `)"
if [[ -z "$CMD" ]]; then
  log_hook "skip-no-command"
  exit 0
fi
...
case "$CMD" in
  ccp|ccp\ *)
    log_hook "skip-already-prefixed"
    exit 0
    ;;
esac
