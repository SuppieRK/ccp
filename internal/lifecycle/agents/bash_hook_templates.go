...
rewrite_segment() {
  ...
  printf '%scmdshape %s' "${segment:0:start}" "${segment:start}"
}
...
func bashRewriteHookScriptContent(agent, logFile string) string {
	return "#!/bin/bash\n" +
		"# generated by cmdshape init for " + agent + "\n" +
		"# " + hookAgentTitle(agent) + " PreToolUse hook: rewrite shell commands to use cmdshape.\n\n" +
		"LOG_FILE=\"${TMPDIR:-/tmp}/" + logFile + "\"\n\n" +
		"log_hook() {\n  printf '%s\\n' \"$1\" >>\"$LOG_FILE\"\n}\n" +
		bashJSONHelpers() +
		bashRewriteHelpers() +
		`
if ! command -v cmdshape >/dev/null 2>&1; then
  log_hook "skip-no-cmdshape"
  exit 0
fi
...
printf '%s\n' "{\"hookSpecificOutput\":{\"hookEventName\":\"PreToolUse\",\"permissionDecision\":\"allow\",\"permissionDecisionReason\":\"cmdshape auto-rewrite\",\"updatedInput\":{\"command\":\"$ESCAPED_CMD\"}}}"
`
}
