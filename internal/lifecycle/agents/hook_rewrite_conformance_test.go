...
var hookRewriteConformanceCases = []hookRewriteConformanceCase{
	...
	{name: "quoted pipe", command: `git log --format='a|b'`, expected: `cmdshape git log --format='a|b'`},
	{name: "escaped quote", command: `git commit -m "say \"hi\""`, expected: `cmdshape git commit -m "say \"hi\""`},
	{name: "leading assignments", command: `FOO=1 BAR='two words' git status`, expected: `FOO=1 BAR='two words' cmdshape git status`},
	{name: "env and sudo prefixes", command: `env -i FOO=1 sudo -u root git status`, expected: `env -i FOO=1 sudo -u root cmdshape git status`},
	{name: "sudo and env prefixes", command: `sudo --user=root env FOO=1 git status`, expected: `sudo --user=root env FOO=1 cmdshape git status`},
	{name: "already prefixed", command: "cmdshape git status", expected: "cmdshape git status"},
	{name: "mixed prefixed chain", command: "cmdshape git status && ls", expected: "cmdshape git status && cmdshape ls"},
	...
	ginkgo.It("classifies every Bash hook command with the shared corpus", func() {
		for _, testCase := range hookRewriteConformanceCases {
			...
			result := runHookScript(
				ginkgo.GinkgoT(),
				"cmdshape-rewrite.sh",
				"cmdshape-rewrite.log",
				bashRewriteHookScriptContent("conformance", "cmdshape-rewrite.log"),
				input.String(),
				true,
			)
