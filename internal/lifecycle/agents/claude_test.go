...
var _ = ginkgo.Describe("Claude hook script execution", func() {
	...
	ginkgo.DescribeTable("executing the rewrite hook",
		...
		ginkgo.Entry("empty rewrite from whitespace command", hookCase{name: "empty rewrite from whitespace command", input: `{"tool_input":{"command":"   "}}`, withCCP: true, wantLog: "skip-empty-rewrite", wantNoOutput: true}),
		...
		ginkgo.Entry("invalid rewritten shell", hookCase{name: "invalid rewritten shell", input: `{"tool_input":{"command":"pwd &&"}}`, withCCP: true, wantLog: "skip-invalid-shell", wantNoOutput: true}),
		ginkgo.Entry("simple chained rewrite", hookCase{name: "simple chained rewrite", input: `{"tool_input":{"command":"pwd && ls"}}`, withCCP: true, wantCommand: "ccp pwd && ccp ls"}),
		ginkgo.Entry("double quoted rewrite", hookCase{name: "double quoted rewrite", input: `{"tool_input":{"command":"echo \"test\""}}`, withCCP: true, wantCommand: `ccp echo "test"`}),
		ginkgo.Entry("single quoted rewrite", hookCase{name: "single quoted rewrite", input: `{"tool_input":{"command":"echo 'test'"}}`, withCCP: true, wantCommand: `ccp echo 'test'`}),
		ginkgo.Entry("backslash command rewrite", hookCase{name: "backslash command rewrite", input: `{"tool_input":{"command":"echo foo\\ bar"}}`, withCCP: true, wantCommand: `ccp echo foo\ bar`}),
		ginkgo.Entry("command substitution rewrite", hookCase{name: "command substitution rewrite", input: `{"tool_input":{"command":"echo $(pwd)"}}`, withCCP: true, wantCommand: `ccp echo $(pwd)`}),
		ginkgo.Entry("parameter expansion rewrite", hookCase{name: "parameter expansion rewrite", input: `{"tool_input":{"command":"echo ${HOME}"}}`, withCCP: true, wantCommand: `ccp echo ${HOME}`}),
		ginkgo.Entry("heredoc token rewrite", hookCase{name: "heredoc token rewrite", input: `{"tool_input":{"command":"cat <<EOF"}}`, withCCP: true, wantCommand: `ccp cat <<EOF`}),
		...
		ginkgo.Entry("mixed prefixed chain rewrite", hookCase{name: "mixed prefixed chain rewrite", input: `{"tool_input":{"command":"ccp pwd && ls | env ; whoami || date"}}`, withCCP: true, wantCommand: "ccp pwd && ccp ls | ccp env ; ccp whoami || ccp date"}),
	)
})
