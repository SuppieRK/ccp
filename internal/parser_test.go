...
var _ = Describe("ParseCommandLine", func() {
	DescribeTable("parses shell-like command lines",
		...
		Entry("backslashes outside quotes escape the next rune literally",
			`cmd one\ two\ three --path C:\temp\file.txt`,
			contracts.Command{
				RawInput: `cmd one\ two\ three --path C:\temp\file.txt`,
				Args:     []string{"cmd", "one two three", "--path", "C:tempfile.txt"},
				Tool:     "cmd",
			},
		),
	)
